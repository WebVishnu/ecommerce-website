...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    <div
      className="relative border rounded-lg p-4 sm:p-6 shadow-lg transition-transform duration-700 overflow-hidden group"
    ...
    >
      ...
      <h3 className="mt-4 text-sm sm:text-lg font-medium transition-colors duration-300 hover:text-my-blue text-center">
        {name}
      </h3>
      ...
      {/* Quick Add Modal */}
      <QuickAddModal isOpen={isQuickAddModalOpen} onClose={() => setIsQuickAddModalOpen(false)} product={product} />
    </div>
  );
};
